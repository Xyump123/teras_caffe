                    ...
                    <tr class="item-row">
                        ...
                        <td>
                            <select name="items[0][level_pedas]" class="form-control">
                                ...
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <option value="<?= $i ?>">🌶️ Level <?= $i ?></option>
                                <?php endfor; ?>
                            </select>
                        </td>
...
<script>
    ...
    document.getElementById('tambahItem').addEventListener('click', function() {
        ...
        <?php foreach ($menu as $m): ?>
            menuOptions += `<option value="<?= $m['id'] ?>" data-harga="<?= $m['harga'] ?>" data-stok="<?= $m['stok'] ?>">
                <?= addslashes($m['nama_menu']) ?> (Stok: <?= $m['stok'] ?>) - Rp <?= number_format($m['harga'], 0, ',', '.') ?>
            </option>`;
        <?php endforeach; ?>
